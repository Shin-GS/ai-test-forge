...
class AgentLoopServiceTest {
    ...
    @Mock
    private AiService reasoningAiService;

    @Mock
    private AiService fastAiService;
    ...
    @Mock
    private SseEventBufferService sseEventBufferService;
    ...
    void setUp() {
        agentLoopService = new AgentLoopService(
                reasoningAiService, fastAiService, chatService, messageRepository, sessionRepository,
                specRepository, specToolConverter, specControlFilter,
                workspaceService, twoStageFilterService, settingsService, sseEventBufferService
        );
    ...
    class StartLoop {
        ...
        void fail_concurrent_limit_exceeded_sends_e004_error() {
            // given
            Long sessionId = 1L;
            ...
            // then — error SSE 이벤트 전송 검증
            then(sseEventBufferService).should().publish(eq(sessionId), eq("error"), any());
            then(sseEventBufferService).should().publish(eq(sessionId), eq("done"), any());
            then(sseEventBufferService).should().cleanup(sessionId);
        ...
        void success_ai_text_response_sends_message_and_done() {
            // given
            Long sessionId = 1L;
            ...
            given(reasoningAiService.chat(anyList(), anyList())).willReturn(aiResponse);
            ...
            then(sseEventBufferService).should().publish(eq(sessionId), eq("message"), any());
            then(sseEventBufferService).should().publish(eq(sessionId), eq("done"), any());
            then(sseEventBufferService).should().cleanup(sessionId);
        }
        ...
        void success_ai_tool_call_sends_tool_call_start() {
            // given
            Long sessionId = 1L;
            ChatSession session = createChatSession(sessionId, 100L);
            ...
            then(sseEventBufferService).should().publish(eq(sessionId), eq("tool_call_start"), any());
            // emitter가 complete 되지 않음 (tool result를 기다림)
            then(sseEventBufferService).should(never()).cleanup(sessionId);
        }
    }
    ...
    class HandleToolResult {
        ...
        void success_tool_result_then_ai_text_completes_loop() {
            // given
            Long sessionId = 1L;
            ChatSession session = createChatSession(sessionId, 100L);
            ...
            SettingsResponse settingsResponse = new SettingsResponse("openai", "gpt-4o", "openai", "gpt-4o-mini", 20, 5, 120, false);
    ...
    class CheckConstraints {
        ...
        void fail_max_iterations_exceeded_sends_error() {
            // given
            Long sessionId = 1L;
