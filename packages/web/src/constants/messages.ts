export const MESSAGES = {
  common: {
    loading: '로딩 중...',
    error: '오류가 발생했습니다',
    retry: '다시 시도',
    confirm: '확인',
    cancel: '취소',
    delete: '삭제',
    save: '저장',
    create: '생성',
    close: '닫기',
    unknownError: '알 수 없는 오류가 발생했습니다.',
    fetchError: '데이터를 불러올 수 없습니다',
  },
  auth: {
    loginTitle: 'AI Test Forge',
    loginSubtitle: '테스트 데이터 생성 플랫폼',
    emailLabel: '이메일',
    emailPlaceholder: 'name@company.com',
    passwordLabel: '비밀번호',
    passwordPlaceholder: '비밀번호 입력',
    loginButton: '로그인',
    loginLoading: '로그인 중...',
    loginFailed: '이메일 또는 비밀번호가 올바르지 않습니다.',
    logout: '로그아웃',
  },
  chat: {
    newChat: '+ 새 대화',
    inputPlaceholder: '메시지를 입력하세요...',
    inputLoading: 'AI 응답 대기 중...',
    sendButton: '전송',
    onboardingTitle: '🚀 빠른 시작',
    onboardingHint: '또는 자유롭게 입력하세요...',
    sessionEmpty: '대화를 시작해보세요',
  },
  subdomain: {
    title: '서브도메인',
    searchPlaceholder: '🔍 서버 검색...',
    loading: '서브도메인 목록을 불러오는 중...',
    emptyTitle: '등록된 서브도메인이 없습니다',
    emptyDescription:
      '서브도메인 서버에 클라이언트 라이브러리를 추가하면 자동으로 등록됩니다.',
    integrationGuide: '연동 가이드 보기',
    goToChat: '채팅에서 등록하기',
    staleWarning: (count: number) =>
      `⚠️ ${count}개 서버의 연결이 불안정합니다`,
    chatButton: '💬 채팅에서 사용',
    noSearchResult: (query: string) =>
      `"${query}"에 대한 검색 결과가 없습니다.`,
  },
  recipe: {
    title: '레시피',
    searchPlaceholder: '🔍 레시피 검색...',
    loading: '레시피 목록을 불러오는 중...',
    emptyTitle: '저장된 레시피가 없습니다',
    emptyDescription:
      '채팅에서 작업 후 "레시피로 저장해줘"라고 말해보세요.',
    goToChat: '💬 채팅으로 이동',
    run: '▶ 실행',
    detail: '상세 보기',
    frequentTitle: '⭐ 자주 사용',
    allTitle: '전체',
    noSearchResult: (query: string) =>
      `"${query}"에 대한 검색 결과가 없습니다.`,
  },
  settings: {
    title: '설정',
    workspace: {
      title: '워크스페이스',
      createButton: '+ 새 워크스페이스',
      creating: '생성 중...',
      deleting: '삭제 중...',
      namePlaceholder: '워크스페이스 이름',
      default: '기본',
      loading: '워크스페이스를 불러오는 중...',
      loadError: '워크스페이스를 불러올 수 없습니다.',
      empty: '등록된 워크스페이스가 없습니다.',
      emptyMappings: '매핑된 서브도메인이 없습니다.',
      createFailed: '생성에 실패했습니다.',
      confirmDelete: (name: string) =>
        `"${name}" 워크스페이스를 삭제하시겠습니까?`,
    },
    ai: {
      title: 'AI 설정',
      reasoningTitle: '🧠 Reasoning 모델 (고성능)',
      fastTitle: '⚡ Fast 모델 (경량)',
      providerLabel: 'Provider',
      modelLabel: 'Model',
      readonlyNotice: 'AI 모델 설정은 서버 배포 설정(yml/env)에서만 변경 가능합니다',
      nextActionLabel: '다음 액션 힌트',
      nextActionDescription: '작업 완료 후 다음으로 할 수 있는 것을 제안 (AI 토큰 추가 사용)',
      loading: 'AI 설정을 불러오는 중...',
      loadError: 'AI 설정을 불러올 수 없습니다.',
    },
    agentLoop: {
      title: 'Agent Loop 설정',
      maxIterationsLabel: '최대 반복 횟수',
      maxIterationsUnit: '회',
      maxToolCallsLabel: '턴당 Tool Call',
      maxToolCallsUnit: '개',
      timeoutLabel: '타임아웃',
      timeoutUnit: '초',
      saveButton: '저장',
      saved: '저장됨 ✓',
      saveFailed: '저장 실패',
    },
    account: {
      title: '계정',
    },
  },
} as const
